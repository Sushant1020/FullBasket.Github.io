...
module.exports.home = (('/products', catchAsync(async (req, res) => {
  ...
  for (prode of product) {
    if (prode.category === 'mobile')
      prod.push(prode);
  }
  for (prode of product) {
    if (prode.category === 'clothes')
      prodc.push(prode);
  }
  for (prode of product) {
    if (prode.category === 'food')
      prodf.push(prode);
  }
  for (prode of product) {
    if (prode.category === 'electronics')
      prodel.push(prode);
  }
  for (prode of product) {
    if (prode.category === 'tv')
      prodt.push(prode);
  }
  ...
  res.render('products/home', { product, currentUser, rand, prod, prodc, prodf, prodel, prodt });
})));
...
module.exports.searchresult = ('/search', async (req, res) => {
